...
Canva 实操步骤
① 画"厚度"层（最底下）
插入一个圆角矩形，填深紫色（比如 #7B3FD4）
这是按钮的"底座"
② 画主体层
复制这个圆角矩形，改成亮紫色（比如 #A468F5）
叠在底座上，往上移 8~12px（按钮高度的 8~10% 左右）
这样底部就露出一条深色边 → 厚度感出来了
③ 加顶部高光
再画一个更扁的圆角矩形，放在主体顶部内侧
填白色，透明度调到 20~30%（或直接用更浅的紫 #C094FF）
只占按钮上面 1/3 左右的高度，宽度略窄于主体
④ 整体描边
选中主体圆角矩形 → 边框样式 → 加一圈深紫或深蓝描边（3~5px）
你参考图里就有一圈很深的外描边，这让按钮从背景里"跳"出来
⑤ 文字也要立体
Canva 文字特效里选 "拼接"（Splice） 或 "空心"+手动叠一层
你参考图的"建造"二字 = 白字 + 深紫描边 + 轻微下移的深色投影，用"拼接"效果调一下偏移和颜色就很接近